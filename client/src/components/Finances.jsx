...
const Finances = () => {
  ...
  const totalExpenses = calculateTotal('day', )
  const totalPrice = calculateTotal('day', events)
  console.log('totalexpense', totalExpenses)
  console.log('totalprice', totalPrice)

  // Revenues
  const totalYearPrice = calculateTotal('year', events.price)
  const totalMonthPrice = calculateTotal('month', events.price)
  const totalWeekPrice = calculateTotal('week', events.price)
  const totalDayPrice = calculateTotal('day', events.price)

  // Expenses
  const totalYearExpense = calculateTotal('year', expenses.amount)
  const totalMonthExpense = calculateTotal('month', expenses.amount)
  const totalWeekExpense = calculateTotal('week', expenses.amount)
  const totalDayExpense = calculateTotal('day', expenses.amount)

  return (
    <div className="py-2">
      <Link to="/expenses">
        <button className="text-white bg-blue-900 px-2 py-1 ml-6 rounded-lg font-semibold">
          Expenses
        </button>
      </Link>
      ...
      <h3 className="text-center font-bold text-2xl mt-8 mb-4 text-blue-900">Total Expenses</h3>
